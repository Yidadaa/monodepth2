...
def test_simple(args):
    ...
    atten_path = os.path.join(model_path, "attention.pth")
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            ref_image = pil.open(image_path).convert('RGB')
            original_width, original_height = ref_image.size
            ref_image = ref_image.resize((feed_width, feed_height), pil.LANCZOS)
            ref_image = transforms.ToTensor()(ref_image).unsqueeze(0)
            ref_image = ref_image.to(device)
            ref_features = encoder(ref_image)

            features[-1], ref_features[-1], _, __ = attention(features[-1], features[-1])
